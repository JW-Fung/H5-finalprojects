(function($) {
	...
	$.fn.caroufredsel = $.fn.carouFredSel = function(u, w) {
		...
		y._cfs_init = function(o, a, b) {
			...
			if (opts.responsive) {
				...
				switch (opts.align) {
					...
					default:
						opts.align = false;
						opts.usePadding = (opts.padding[0] == 0 && opts.padding[1] == 0 && opts.padding[2] == 0 && opts.padding[3] ==
							0) ? false : true;
						break
				}
			}
			...
			if (is_undefined(opts.scroll.items)) {
				opts.scroll.items = (opts.responsive || opts.items.visibleConf.variable || opts.items.filter != '*') ? 'visible' :
					opts.items.visible
			}
		...
		y._cfs_build = function() {
			...
			var a = y.children(),
				orgCSS = in_mapCss(y, ['textAlign', 'float', 'position', 'top', 'right', 'bottom', 'left', 'zIndex', 'width',
		...
		y._cfs_bind_events = function() {
			...
			y.bind(cf_e('slide_prev', conf), function(e, b, c) {
				...
				opts.items.visibleConf.old = opts.items.visible;
				if (opts.items.visibleConf.variable) {
					var g = cf_getItemsAdjust(gn_getVisibleItemsNext(d, opts, itms.total - c), opts, opts.items.visibleConf.adjust,
						$tt0);
					if (opts.items.visible + c <= g && c < itms.total) {
						c++;
						g = cf_getItemsAdjust(gn_getVisibleItemsNext(d, opts, itms.total - c), opts, opts.items.visibleConf.adjust,
							$tt0)
					}
			...
			y.bind(cf_e('slide_next', conf), function(e, c, d) {
				...
				opts.items.visibleConf.old = opts.items.visible;
				if (opts.items.visibleConf.variable) {
					var i = cf_getItemsAdjust(gn_getVisibleItemsNextTestCircular(f, opts, d, h), opts, opts.items.visibleConf.adjust,
						$tt0);
					while (opts.items.visible - d >= i && d < itms.total) {
						d++;
						i = cf_getItemsAdjust(gn_getVisibleItemsNextTestCircular(f, opts, d, h), opts, opts.items.visibleConf.adjust,
							$tt0)
					}
